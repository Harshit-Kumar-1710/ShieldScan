...
function exportCSV(rows) {
  const headers = ["#", "Timestamp", "Mode", "Risk Level", "Is XSS", "Confidence", "Analyst Feedback", "Payload"];
  const lines = rows.map((r, i) => [
    ...
    r.analyst_feedback || "unreviewed",
    `"${(r.payload || "").replace(/"/g, '""')}"`,
  ].join(","));
  const blob = new Blob([[headers.join(","), ...lines].join("\n")], { type: "text/csv" });
  const a = Object.assign(document.createElement("a"), {
    href: URL.createObjectURL(blob),
    download: `shieldscan_history_${Date.now()}.csv`,
  });
  a.click();
  URL.revokeObjectURL(a.href);
}

function exportSingleJson(entry) {
  const reportData = {
    scan_id: `SS-${entry.id || Date.now()}`,
    timestamp: entry.timestamp,
    payload: entry.payload,
    model: {
      name: "CalibratedLinearSVC",
      version: "1.1.0",
      engine: "Character TF-IDF + 18 Handcrafted Features",
    },
    ml_result: {
      is_xss: entry.is_xss,
      xss_probability: entry.confidence,
      severity: entry.risk_level ? entry.risk_level.toUpperCase() : (entry.is_xss ? "HIGH" : "SAFE"),
      mode: entry.mode || "single",
    },
    analyst_feedback: entry.analyst_feedback || "unreviewed",
    disclaimer: "ShieldScan is a pre-execution screening layer. Contextual output encoding and sanitization are required.",
  };
  ...
  a.href = url;
  a.download = `shieldscan_scan_${entry.id || Date.now()}.json`;
  a.click();
...
function DetailDrawer({ entry, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="rounded-2xl w-full max-w-lg animate-fsu"
      ...
      >
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid var(--border)" }}>
          <div className="flex items-center gap-2">
            ...
            <p className="font-bold text-sm" style={{ color: "var(--text-primary)" }}>Scan Record Detail</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => exportSingleJson(entry)}
              className="btn-ghost flex items-center gap-1 text-xs px-2.5 py-1"
              title="Export scan record as JSON"
            >
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: "Confidence", value: entry.confidence != null ? `${(entry.confidence * 100).toFixed(1)}%` : "—" },
              {
                ...
                value: entry.analyst_feedback === "correct" ? (
                  <span className="text-emerald-400 font-bold flex items-center gap-1">
                    <ThumbsUp size={11} /> Correct
                  </span>
                ) : entry.analyst_feedback === "incorrect" ? (
                  <span className="text-red-400 font-bold flex items-center gap-1">
                    <ThumbsDown size={11} /> Incorrect
                  </span>
                ) : (
                  <span style={{ color: "var(--text-muted)" }}>Unreviewed</span>
                ),
              },
              ...
              <div key={r.label} className="card-subtle p-3">
                ...
                <div className="text-sm font-semibold" style={{ color: r.color || "var(--text-primary)" }}>{r.value}</div>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <code
              className="block text-xs font-mono rounded-xl p-3 break-all leading-relaxed max-h-36 overflow-y-auto"
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          Payloads analyzed in this browser. Feedback is collected for future offline benchmark reviews.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        >
          ...
          {activeFilters > 0 && (
            <span className="text-white text-xs rounded-full w-4 h-4 flex items-center justify-center font-bold" style={{ background: "var(--accent)" }}>
              {activeFilters}
            </span>
          )}
        </button>
        <button
          ...
          className="btn-ghost flex items-center gap-1.5 text-sm px-3 py-2 disabled:opacity-40"
        >
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-3 mb-4 p-4 rounded-xl animate-fsu" style={{ background: "var(--bg-subtle)", border: "1px solid var(--border)" }}>
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {[["all", "All"], ["xss", "XSS Only"], ["safe", "Safe Only"]].map(([v, l]) => (
              <FilterBtn key={v} active={filterXss === v} onClick={() => { setFilterXss(v); setPage(1); }}>{l}</FilterBtn>
            ))}
          </div>
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-xs font-semibold" style={{ color: "var(--text-muted)" }}>Mode:</span>
            {[["all", "All"], ["single", "Single"], ["batch", "Batch"]].map(([v, l]) => (
              <FilterBtn key={v} active={filterMode === v} onClick={() => { setFilterMode(v); setPage(1); }}>{l}</FilterBtn>
            ))}
          </div>
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-xs font-semibold" style={{ color: "var(--text-muted)" }}>Risk:</span>
            {riskLevels.map(v => (
              <FilterBtn key={v} active={filterRisk === v} onClick={() => { setFilterRisk(v); setPage(1); }}>
      ...
      {rawHistory.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4 text-xs">
          {[
            ...
            <span key={p.v} className="rounded-full px-3 py-1 font-medium" style={{ background: p.bg, color: p.color, border: `1px solid ${p.border}` }}>
              {p.v}
            </span>
          ))}
        </div>
      ...
      {rawHistory.length > 0 && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16" style={{ color: "var(--text-muted)" }}>
          ...
          <button
            onClick={() => { setSearch(""); setFilterXss("all"); setFilterMode("all"); setFilterRisk("all"); }}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead style={{ background: "var(--bg-subtle)", borderBottom: "1px solid var(--border)" }}>
                <tr>
                  {["#", "Payload", "Result", "Risk", "Confidence", "Analyst Feedback", "Mode", "Time"].map(h => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-bold" style={{ color: entry.is_xss ? "#f87171" : "#34d399" }}>
                          {entry.is_xss ? "XSS" : "Safe"}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <SeverityBadge level={entry.risk_level || (entry.is_xss ? "high" : "safe")} size="sm" />
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs">
                      {entry.analyst_feedback === "correct" ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-emerald-400 font-semibold" style={{ background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.2)" }}>
                          <ThumbsUp size={10} /> Correct
                        </span>
                      ) : entry.analyst_feedback === "incorrect" ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-red-400 font-semibold" style={{ background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.2)" }}>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs rounded-md px-1.5 py-0.5 capitalize" style={{ background: "var(--bg-subtle)", color: "var(--text-muted)", border: "1px solid var(--border)" }}>
                        {entry.mode || "single"}
                      </span>
                    </td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: "1px solid var(--border)", background: "var(--bg-subtle)" }}>
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-1.5 rounded-lg transition-all disabled:opacity-30 btn-ghost">
                  <ChevronLeft size={15} />
                </button>
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => Math.max(1, Math.min(totalPages - 4, page - 2)) + i).map(pg => (
                  <button
                    ...
                    style={{ background: pg === page ? "var(--accent)" : "transparent", color: pg === page ? "white" : "var(--text-muted)" }}
                  >
                    {pg}
                  </button>
                ))}
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-1.5 rounded-lg transition-all disabled:opacity-30 btn-ghost">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}>
          <div className="rounded-2xl p-6 max-w-sm w-full animate-fsu" style={{ background: "var(--bg-card)", border: "1px solid var(--border)", boxShadow: "var(--shadow-md)" }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="font-bold" style={{ color: "var(--text-primary)" }}>Clear all history?</p>
            </div>
            <p className="text-sm mb-5" style={{ color: "var(--text-secondary)" }}>
              This will permanently delete all {rawHistory.length} scan records. This cannot be undone.
            </p>
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  localStorage.removeItem("shieldscan_history");
                  setRawHistory([]);
